...
#mesh.link = np.atleast_2d([1,1,1], dtype=np.int32) # make sure it's a n x 3 matrix
...
p0[selection] = P0/1e3           #assigning a pressure value to the selected nodes within the cylindrical vol
...
JT_full, dataT, _ = ff.inverse.jacobian_stnd_CW(meshT, normalize=0, mus =True)
print(JT_full.shape)
JT_mus = JT_full[:,:1728000]
JT_mua = JT_full[:,1728000:]
...
# J_ac and J_md
J_ac_mua = J0_mua - JT_mua
J_ac_mus = J0_mus - JT_mus

# It should be J_AC, not J(T/2)! There's an error in the paper
J_md_mua = (J_ac_mua*y_dc[:,None] - J0_mua*y_ac[:,None])/y_dc[:,None]**2
J_md_mus = (J_ac_mus*y_dc[:,None] - J0_mus*y_ac[:,None])/y_dc[:,None]**2

# %%
# plot the DC banana
banana1 = J0_mus.reshape((xgrid.size, ygrid.size, zgrid.size), order='f') 
...
norm = mcolors.Normalize(vmin=0, vmax=0.5e-8)  
...
# %%
banana_ac = J_ac_mus.reshape((xgrid.size, ygrid.size, zgrid.size), order='f') 
...
cmap = plt.get_cmap('hot')  
norm = mcolors.Normalize(vmin=0, vmax=0.2e-8)  
...
banana_md = J_md_mus.reshape((xgrid.size, ygrid.size, zgrid.size), order='f')


norm = mcolors.TwoSlopeNorm(vmin=-0.7e-4, vcenter=0, vmax=0.2e-3)
